...
func batchAddVertex(client goClient.SessionClient) error {
	...
	err := client.BatchSubmit(ctx.Background(), func(ctx ctx.Context, shell goClient.ClientShell) error {
...
Loop:
	for {
		select {
		case <-ticker.C:
			if results, err := client.SubmitScript(ctx.Background(), "g.V().count()"); err == nil {
