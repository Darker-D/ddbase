...
func main() {
	...
	client.BatchSubmit(ctx.Background(), func(ctx ctx.Context, c goClient.ClientShell) error {
		...
		_, err := c.SubmitScriptBound(ctx, dsl, bindings)
		if err != nil {
			return err
		}
	...
	client.BatchSubmit(ctx.Background(), func(ctx ctx.Context, c goClient.ClientShell) error {
		results, err := c.SubmitScript(ctx, "g.V().id()")
		if err != nil {
			return err
		}
		...
		results, err = c.SubmitScript(ctx, "g.E().id()")
		if err != nil {
			return err
		}
		for _, r := range results {
	...
	client.BatchSubmit(ctx.Background(), func(ctx ctx.Context, c goClient.ClientShell) error {
		_, err := c.SubmitScript(ctx, "g.V().hasLabel('goTest').drop()")
		return err
	})
