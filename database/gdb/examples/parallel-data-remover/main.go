...
func reportCount(client goClient.Client, edge bool, quit chan struct{}) {
	...
	for {
		select {
		case <-ticker.C:
			if results, err := client.SubmitScript(ctx.Background(), dsl); err == nil {
...
func dropElementByIdAsync(client goClient.Client, edge bool, ids []string) goClient.ResultSetFuture {
	...
	future, err := client.SubmitScriptBoundAsync(ctx.Background(), dsl, bindings)
	if err != nil {
...
func main() {
	...
	defer client.Close(ctx.Background())
	...
	if label == "" {
		...
		results, err := client.SubmitScript(ctx.Background(), fetchLabelsDsl)
		if err != nil {
