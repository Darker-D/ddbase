...
func (cn *ConnWebSocket) readResponse() {
	...
	for {
		...
		if response != nil {
			cn.handleResponse(context.TODO(), response)
		}
...
func (cn *ConnWebSocket) handleResponse(ctx context.Context, response *graphsonv3.Response) {
	if response.Code == graphsonv3.RESPONSE_STATUS_AUTHENTICATE {
		...
		cn.SubmitRequestAsync(ctx, request)
		return
	}
...
func (cn *ConnWebSocket) SubmitRequestAsync(ctx context.Context, request *graphsonv3.Request) (*graphsonv3.ResponseFuture, error) {
	if cn.brokenOrClosed() {
	...
	if ok {
		...
		timeout = cn.opt.WriteTimeout
	}

	// send request to server
	cn.wLock.Lock()
	if err = cn.netConn.SetWriteDeadline(cn.deadline(timeout)); err == nil {
