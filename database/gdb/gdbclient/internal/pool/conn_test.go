...
func TestNewConnWebSocket(t *testing.T) {
	...
	Convey("send request and wait response", t, func() {
		...
		Convey("send one request and wait", func() {
			request, _ := graphsonv3.MakeRequestWithOptions(ctx.Background(), "g.V().count()", nil)
			respFuture, err := conn.SubmitRequestAsync(ctx.Background(), request)
			So(err, ShouldBeNil)
		...
		Convey("send multi request async and wait", func() {
			request1, _ := graphsonv3.MakeRequestWithOptions(ctx.Background(), "g.V().count()", nil)
			respFuture1, err := conn.SubmitRequestAsync(ctx.Background(), request1)
			So(err, ShouldBeNil)

			request2, _ := graphsonv3.MakeRequestWithOptions(ctx.Background(), "g.E().count()", nil)
			respFuture2, err := conn.SubmitRequestAsync(ctx.Background(), request2)
			So(err, ShouldBeNil)
		...
		Convey("send multi request async over max pending", func() {
			...
			request, _ := graphsonv3.MakeRequestWithOptions(ctx.Background(), "g.V().count()", nil)
			_, err := conn.SubmitRequestAsync(ctx.Background(), request)
		...
		Convey("send options request", func() {
			...
			Convey("send requests with the same requestId", func() {
				request1, _ := graphsonv3.MakeRequestWithOptions(ctx.Background(), "g.V().count()", opt)
				respFuture1, err := conn.SubmitRequestAsync(ctx.Background(), request1)
				So(err, ShouldBeNil)
				resp1 := respFuture1.Get()
				...
				request2, _ := graphsonv3.MakeRequestWithOptions(ctx.Background(), "g.E().count()", opt)
				respFuture2, err := conn.SubmitRequestAsync(ctx.Background(), request2)
				So(err, ShouldBeNil)
				resp2 := respFuture2.Get()
