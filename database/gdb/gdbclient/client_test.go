...
func TestNewClient(t *testing.T) {
	...
	Convey("create new client", t, func() {
		...
		_, err = client.SubmitScript(ctx.Background(), "g.V().count")
	...
	Convey("send multi request and pending", t, func() {
		...
		Convey("multi request and waiting in one routine", func() {
			...
			for i := 0; i < settings.MaxConcurrentRequest*settings.PoolSize; i++ {
				f, err := client.SubmitScriptAsync(ctx.Background(), "g.V().count()")
				So(err, ShouldBeNil)

				futureList = append(futureList, f)
			}

			_, err := client.SubmitScriptAsync(ctx.Background(), "g.V().count()")
...
func TestNewSessionClient(t *testing.T) {
	...
	SkipConvey("create new session client and batch submit", t, func() {
		...
		client.BatchSubmit(ctx.Background(), func(ctx ctx.Context, shell ClientShell) error {
			results, err := shell.SubmitScript(ctx, "g.addV('testV').property('name', 'Luck')")
			So(err, ShouldBeNil)
			id1 := results[0].GetVertex().Id()

			results, err = shell.SubmitScript(ctx, "g.addV('testV').property('name', 'Jack')")
			So(err, ShouldBeNil)
			...
			_, err = shell.SubmitScriptBound(ctx, dsl, bindings)
			So(err, ShouldBeNil)
	...
	SkipConvey("create new session client and fail to submit", t, func() {
		...
		client.BatchSubmit(ctx.Background(), func(ctx ctx.Context, shell ClientShell) error {
			...
			bindings := make(map[string]interface{})
			bindings["GDB___id"] = addVid
			results, err := shell.SubmitScriptBound(ctx, "g.V(GDB___id).count()", bindings)
			So(err, ShouldBeNil)
			So(results[0].GetInt64(), ShouldEqual, 0)
