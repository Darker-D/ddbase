...
var (
	...
	DBSqlCostSeconds = float64TagName("db.sql.CostSeconds")
	DBSqlRows        = int64TagName("db.sql.Rows")
	DBSql            = stringTagName("db.sql")
	DBSqlStack       = stringTagName("db.sql.Stack")
...
type float64TagName string

// Set adds a int64 tag to the `span`
func (tag float64TagName) Set(span opentracing.Span, value float64) {
	span.SetTag(string(tag), value)
}
