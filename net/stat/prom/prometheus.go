...
var (
	...
	// MqClient mq client
	MqClient = New().WithTimer("go_mq_client", []string{"queue"}).WithState("go_mq_state", []string{"method", "name"}).WithCounter("go_mq_code", []string{"queue", "code"})
...
// Prom struct info
type Prom struct {
	timer   *prometheus.HistogramVec //直方图
